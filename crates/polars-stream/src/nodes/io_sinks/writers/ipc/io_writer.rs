...
impl IOWriter {
    pub async fn run(self) -> PolarsResult<()> {
        ...
        let (mut writeable, sync_on_close) = file.await?;

        let mut writer = WriteableWrap {
            writeable: &mut writeable,
        };
...
struct WriteableWrap<'a> {
    writeable: &'a mut Writeable,
}

impl<'a> Deref for WriteableWrap<'a> {
    type Target = Writeable;
...
impl<'a> DerefMut for WriteableWrap<'a> {
    fn deref_mut(&mut self) -> &mut Self::Target {
        self.writeable
    }
}

impl<'a> WriteableWrap<'a> {
